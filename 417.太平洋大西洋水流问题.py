...
class Solution1:
    ...
    def pacificAtlantic(self, heights: list[list[int]]) -> list[list[int]]:
        ...
        @lru_cache(None)
        def dfs(i,j,mode):
            if mode ==1 and (i==0 or j==0):
                return True
            elif mode==2 and (i==n-1 or j==m-1):
                return True
            else:
                used[i][j]=0
                for nx,ny in ((i+1,j),(i-1,j),(i,j+1),(i,j-1)):
                    if 0<=nx<=n-1 and 0<=ny<=m-1 and used[nx][ny] and heights[nx][ny]<=heights[i][j] and dfs(nx,ny,mode):
                        used[i][j]=1
                        return True
                used[i][j]=1
                return False
